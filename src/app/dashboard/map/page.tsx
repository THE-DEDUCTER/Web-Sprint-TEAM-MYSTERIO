...
export default function CampusMap() {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      map.addControl(new (tt as any).NavigationControl())
      ...
      seedMapPois.forEach((poi) => {
        ...
        const popup = new (tt as any).Popup({ offset: 20 }).setHTML(
          `<strong>${poi.name}</strong><br/><span style="font-size:12px;color:#666">${poi.description ?? ''}</span>`,
        )
        const marker = new (tt as any).Marker({ element: el }).setLngLat([poi.lng, poi.lat]).setPopup(popup).addTo(map)
        markers.push(marker)
      })
      setReady(true)
    }
      ...
      {!isTomTomConfigured ? (
        <Card className="p-8 text-center">
          ...
          <p className="mx-auto mt-1.5 max-w-md text-sm text-neutral-500 dark:text-neutral-400">
            Set <code className="rounded bg-neutral-100 dark:bg-neutral-800 px-1.5 py-0.5">NEXT_PUBLIC_TOMTOM_API_KEY</code>{' '}
            in your <code className="rounded bg-neutral-100 dark:bg-neutral-800 px-1.5 py-0.5">.env.local</code> file to load the interactive campus map.
          </p>
