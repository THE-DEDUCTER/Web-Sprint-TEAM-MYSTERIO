...
export default function Communities() {
  ...
  async function subscribeAllInDept(_deptName: string, ids: string[]) {
    if (!user) return
                  ...
                  {list.map((c) => (
                    <Card key={c.id} className="p-4">
                      <div className="flex items-start gap-3">
                        <Link href={`/dashboard/communities/${c.handle}`}>
                          <Avatar name={c.name} url={c.logoUrl} size={44} />
                        </Link>
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-1.5">
                            <Link href={`/dashboard/communities/${c.handle}`} className="truncate font-semibold hover:underline">
                              {c.name}
                            </Link>
