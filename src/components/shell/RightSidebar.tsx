  ...
  return (
    <aside className="hidden xl:flex h-full w-[280px] shrink-0 flex-col gap-4 overflow-y-auto border-l border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-950 p-4">
      ...
      <SidebarWidget title="C/COMMUNITIES" seeAll="/dashboard/communities">
        ...
        {communities.map((c) => (
          <Link key={c.id} href={`/dashboard/communities/${c.handle}`} className="flex items-center gap-2 py-1.5 text-sm hover:text-brand-600">
      ...
      <SidebarWidget title="C/FRIENDS" seeAll="/dashboard/network">
        ...
        {friends.map((f) => (
          <Link key={f.id} href={`/dashboard/profile/${f.handle}`} className="flex items-center gap-2 py-1.5 text-sm hover:text-brand-600">
      ...
      <SidebarWidget title="C/CONNECT">
        ...
        {friends.map((f) => (
          <Link key={f.id} href={`/dashboard/connect?user=${f.handle}`} className="flex items-center justify-between py-1.5 text-sm hover:text-brand-600">
            <span className="truncate">{f.displayName}</span>
  ...
  return (
    <Card className="p-3.5">
      <div className="mb-1 flex items-center justify-between">
        ...
        {seeAll && (
          <Link href={seeAll} className="text-neutral-400 hover:text-brand-600">
            <ArrowRight size={14} />
          </Link>
        )}
      </div>
